...
export default function AiCoachPage() {
  ...
  // Pronunciation state
  const [pronunciationInput, setPronunciationInput] = useState('你好，我想买一瓶矿泉水。');
  ...
  useEffect(() => {
    const key = getGeminiApiKeyFromCookie();
    setApiKey(key);
    setSettings(getStoredSettings());
  }, []);

  const handleCheckGrammar = async () => {
    if (!grammarInput.trim() || !apiKey) return;
    setGrammarLoading(true);
    setGrammarError(null);
    try {
      const res = await checkChineseGrammar(grammarInput, apiKey, settings.language || 'vi');
  ...
  const handleAnalyzePronunciation = async (speechText?: string) => {
    if (!pronunciationInput.trim() || !apiKey) return;
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="space-y-2 border-b border-theme-border/40 pb-4">
        <div className="flex items-center gap-2">
          <span className="px-2.5 py-0.5 rounded-full bg-gradient-to-r from-amber-400 to-rose-400 text-slate-900 font-bold text-xs uppercase tracking-wider">
      ...
      {!apiKey ? (
        <div className="liquid-glass-card rounded-3xl p-6 border border-amber-400/30 bg-amber-500/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Link
            href="/settings"
            className="liquid-glass-btn px-5 py-2.5 rounded-xl text-xs font-semibold flex items-center gap-2 self-start sm:self-auto shrink-0"
          >
        ...
        <div className="px-4 py-2 rounded-2xl liquid-glass border border-emerald-500/30 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2 text-emerald-300">
            <ShieldCheck className="w-4 h-4" />
            <span>API Key đã kích hoạt (Được lưu an toàn trong Cookie trình duyệt)</span>
          </div>
          <Link href="/settings" className="text-theme-text-muted hover:text-white underline">
            Quản lý Key
          </Link>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 border-b border-theme-border/40 pb-2">
        <button
          ...
          className={`px-5 py-2.5 rounded-2xl text-xs font-semibold transition-all ${
            activeTab === 'grammar'
              ...
              : 'liquid-glass text-theme-text-muted hover:text-white'
          }`}
        >
          📝 Kiểm tra & Nâng cấp Ngữ pháp
        </button>
        <button
        ...
        >
          🎙️ Check Phát âm & Biến điệu Thanh điệu
        </button>
      </div>
